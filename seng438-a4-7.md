...
To find equivalent mutations, we looked at the PITEST summary of the DataUtilities.java file. We checked for all the mutations that survived, because equivalent mutations cannot be killed, as they mimic the correct functionality of the program. When looking at these surviving mutations, we checked the conditions and if they seemed to be equivalent, we tested this functionality manually by changing the DataUtilities code itself (we then changed it back afterwards for consistency).
